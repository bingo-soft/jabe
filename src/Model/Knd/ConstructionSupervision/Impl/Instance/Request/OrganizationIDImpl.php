...
class OrganizationIDImpl extends ModelElementInstanceImpl implements OrganizationIDInterface
{
    public static function registerType(ModelBuilder $modelBuilder): void
    {
        $typeBuilder = $modelBuilder->defineType(
            OrganizationIDInterface::class,
            RequestModelConstants::ELEMENT_NAME_ORGANIZATION_ID
        )
        ->namespaceUri(RequestModelConstants::MODEL_NAMESPACE)
        ->instanceProvider(
            new class implements ModelTypeInstanceProviderInterface
            {
                public function newInstance(ModelTypeInstanceContext $instanceContext): OrganizationIDInterface
                {
                    return new OrganizationIDImpl($instanceContext);
                }
            }
        );
